...
class Cityscapes(tvds.VisionDataset):
    ...
    def __init__(
        ...
        self.filename = sorted([
            fname
            for fname in os.listdir(os.path.join(root, self.base_folder, "gt_alltrain"))
            if fname.endswith("_gtFine_color.png")
        ])
        np.random.RandomState(123).shuffle(self.filename)  # legacy order used by ProGAN

    def __getitem__(self, index):
        im = PIL.Image.open(os.path.join(  # noqa
            self.root, self.base_folder, "gt_alltrain", self.filename[index])).convert('RGB')
        
        if self.transform is not None:
            im = self.transform(im)

        # print('****************', im.shape)   # torch.Size([3, 128, 256])
...
ROOT = os.path.expanduser("~/scratch/dataset")
