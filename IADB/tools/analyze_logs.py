    ...
    for i, log_dict in enumerate(log_dicts):
        ...
        for j, metric in enumerate(metrics):
            ...
            for epoch in epochs:
                ...
                if metric in ['mIoU', 'mAcc', 'aAcc', 'decode.loss_ce_val', 'loss_val']: 
                    ## where epoch based runner has been used  ## continuing with this for everyone of the exp
                    # print('>>>>>>>>', epoch, epoch_logs[metric][0]) 
                    plot_epochs.append(epoch)
                    plot_values.append(epoch_logs[metric][0])
                    ## where iter based runner has been used and where "loss_val is multiple times in a epoch" ## exp 3, 6
                    # avg = 0
                    # for idx in range(len(epoch_logs[metric])): 
                    #     if epoch_logs['mode'][idx] == 'train':
                    #         avg += epoch_logs[metric][idx] 
                    #         avg /= 100
                    #         plot_epochs.append(epoch)
                    #         plot_values.append(avg)
                else:
                    for idx in range(len(epoch_logs[metric])):
                        if epoch_logs['mode'][idx] == 'train':
            ...
            if metric in ['mIoU', 'mAcc', 'aAcc', 'decode.loss_ce_val', 'loss_val']:
                ...
                ax.set_xticks(np.arange(min(plot_epochs), max(plot_epochs)+1, 30)) 
                plt.xlabel('epoch')
