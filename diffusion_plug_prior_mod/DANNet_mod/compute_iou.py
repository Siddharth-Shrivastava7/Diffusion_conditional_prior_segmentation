...
def compute_mIoU(gt_dir, pred_dir, devkit_dir=''):
    ...
    # image_path_list = join(devkit_dir, 'cityscapes_val.txt')
    # label_path_list = join(devkit_dir, 'cityscapes_val.txt')
    ...
    # gt_imgs = [join(gt_dir, x.replace('_leftImg8bit.png','_gtFine_labelIds.png')) for x in gt_imgs]
    # pred_imgs = [join(pred_dir, x) for x in pred_imgs] 
    # red_imgs = [join(pred_dir, x.split('/')[-1].replace('_leftImg8bit.png','_leftImg8bit_color.png')) for x in pred_imgs] 
    gt_imgs = [join(gt_dir, x) for x in gt_imgs] 
    pred_imgs = [join(pred_dir, x.split('/')[-1]) for x in pred_imgs]
    
    # print('***********', len(gt_imgs), len(pred_imgs))
