...
dataset_choices = {
    ...
    'cityscapes_coarse_large', 'cityscapes_fine_large', 'cityscapes_fine_train_large', 'cityscapes_fine_train_v_large'}
...
def get_plot_transform(args):
    ...
    elif args.dataset in ('cityscapes_fine_train_large', 'cityscapes_fine_train_v_large'):
        return cityscapes_train_indices_segmentation_to_img
...
def get_data(args):
    ...
    elif args.dataset == 'cityscapes_fine_train_v_large':
        data_shape = (1, 256, 512)
        num_classes = 20 # 19 + 1 (background, uncertain)
        pil_transforms = get_augmentation(args.augmentation, args.dataset,
                                          (256, 512))
        pil_transforms = Compose(pil_transforms)
        train = CityscapesFast(
            split='train', resolution=(256, 512), transform=pil_transforms,
            only_categories=False, train_fine=True)
        test = CityscapesFast(
            split='test', resolution=(256, 512), transform=pil_transforms,
            only_categories=False, train_fine=True)
    
    elif args.dataset == 'cityscapes_fine_train_large':
        data_shape = (1, 128, 256)
        num_classes = 20 # 19 + 1 (background, uncertain)
        pil_transforms = get_augmentation(args.augmentation, args.dataset,
                                          (128, 256))
