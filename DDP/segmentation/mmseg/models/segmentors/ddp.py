...
def get_model_pred_train(img_metas, device):
    if img_metas[0]['filename'].find('DGinstyle')!=-1:
        pred_folder_name = '/raid/ai24resch01002/predictions/robustnet/dginstyle/dginstyle/val/' # now training with respect to RobustNet(GTA) predictions on DGInstyle 
        pred_imgs_ls = []
        pred_logits_ls = []
        for ind in range(len(img_metas)):
            img_name = img_metas[ind]['filename'].split('/')[-1] 
...
def get_model_pred_val(img_metas, device):  ## For RobustNet predictions
    if img_metas[0]['filename'].find('darkzurich')!=-1:
        ...
        raise Exception("Only Darkzurich predictions are supported for validation, for now!")
    return preds, preds_logits
