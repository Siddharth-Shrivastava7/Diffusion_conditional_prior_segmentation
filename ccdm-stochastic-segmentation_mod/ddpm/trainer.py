...
## replacing idist.device to torch.device
torch_device = torch.device('cuda:5' if torch.cuda.is_available() else 'cpu')
...
def load(filename: str, trainer: Trainer, engine: Engine, weights_only: bool = False):
    ...
    state = torch.load(filename, map_location=torch_device)
...
def run_train(local_rank: int, params: dict):
    ...
    trainer = Trainer(polyak, optimizer, lr_scheduler, class_weights.to(torch_device), debug_checkpoint,
